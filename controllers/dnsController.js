...
exports.postEditDnsCAA = async (req, res) => {
  try {
    ...
    // Felder aus dem Formular
    const { hostname, value, flag, caType, ttl = 3600, disabled = false } = req.body;
...
exports.postEditDnsCNAME = async (req, res) => {
  try {
    ...
    const { domain, recordId } = req.params;
    // Felder aus dem Formular
    const { hostname, destination, ttl, disabled = false } = req.body;
...
exports.postEditDnsMX = async (req, res) => {
  try {
    ...
    const { domain, recordId } = req.params;
    // Felder aus dem Formular
    const { hostname, destination, priority, ttl, disabled = false } = req.body;
...
exports.postEditDnsSRV = async (req, res) => {
  try {
    ...
    const { service, protocol, hostname, destination, priority, weight, port, ttl, disabled = false } = req.body;
